...
class Signature:
    ...
    def sign(self, pcap_file, ignore_ssid = False,only_wildcad= True):
            ...
            if isinstance(pcap_file, str) or isinstance(pcap_file, sc.PacketList):
                ...
                if only_wildcad:
                    if (pcap_file.haslayer(sc.Dot11Elt) and pcap_file[sc.Dot11Elt].ID == 0 and pcap_file[sc.Dot11Elt].info == b'') or ignore_ssid:
                        ...
                        return False
                if not only_wildcad:
                    if pcap_file.haslayer(sc.Dot11ProbeReq) or ignore_ssid:
                        self.signature = self.__extract_signature(pcap_file)
                        return True
    ...
    def match(self, sign_frame, expand = False, weighted = True, lambda_ = 5,key_match='union'):
        ...
        keys_set = ['ht','vendor_specific','extended']
        ...
        try:
            ...
            if key_match == 'union':
                ...
                all_keys = set(self.signature.keys()).union(temp_signature.keys())
                keys = {key for key in all_keys if key in keys_set}
            else:
   ...
   ### new match function
    ...
    def calc_score(self,temp_signature,signatures_wt,keys,lambda_=2.1):
        ...
        for key in keys:
                try:
                    if (key in self.signature) and (key in temp_signature):
                        ...
                        prob_occ = None
                        print('here')
                        print(temp_signature[key])
                        print(signatures_wt.signature)
                        if str(temp_signature[key]) in signatures_wt.signature[key]['attrs']:
                            print('h2')
                            prob_occ = signatures_wt.signature[key]['attrs'][str(temp_signature[key])]
                        if prob_occ:
                            print('h3')
                            weight = weight*signatures_wt.signature[key]['weight'] *prob_occ
                        else:
                            print('h4')
        ...
        return weighted_avg, output_dictinoary, output_list

    def match_wp(self, sign_frame, expand = False, weighted = True, lambda_ = 5,key_match='union',signatures_wt=None):
        ...
        try:
            ...
            if key_match == 't3':
                print('t3 match')
                int_keys  = set(self.signature.keys()).intersection(temp_signature.keys()) 
                t3_keys =  {key for key in int_keys if key in keys_set}
                if not len(t3_keys):
                    print(keys)
                    u_socre, _, il = self.calc_score(temp_signature,signatures_wt,keys,lambda_)
                    i_score, _, il = self.calc_score(temp_signature,signatures_wt,int_keys,lambda_)
                    final_score = (i_score*0.9) + (u_socre*0.1)
                    # final_score = i_score
                    # print(u_socre,i_score,final_score)
                    return il, final_score
                else:
                    print(int_keys)
                    # u_socre, _, il = self.calc_score(temp_signature,signatures_wt,keys,lambda_)
                    t3_score, _, tl = self.calc_score(temp_signature,signatures_wt,int_keys,lambda_)
                    # final_score = (t3_score*0.9) + (u_socre * 0.1)
                    final_score = t3_score
                    # print(t3_score,intersect_score,final_score)
                    return  tl, final_score
            for key in keys:
                try:
                    if (key in self.signature) and (key in temp_signature):
                        weight = self.gamma(key, self.signature[key], temp_signature[key], lambda_= lambda_ )
            ...
            print(self.name)
            print(self.signature)
            print('sig input -> {}'.format(temp_signature))
            # print(output_dictinoary)
            # print(output_list)
            sys.exit()
...
class SignatureBase:
    def __init__(self, stats, **kwargs):
        ...
        try:
            self.signatures = kwargs['signs']
        except KeyError:
            self.signatures = []
            
        try:
            self.db_path = kwargs['path']
        except KeyError:
            self.db_path = ""
    ...
    def explore(self, pcap_file, threshold = 0.85):
        """
    ...
    def predict(self, signature, lambda_ = 2.1, verbose = False,key_match='union',probability_occ = False,expand=False):
        ...
        for index,temp_signature in enumerate(self.signatures):
            if probability_occ:
                ...
                out, score = temp_signature.match_wp(signature, lambda_ = lambda_,key_match= key_match,signatures_wt=self.signatures_wp[index],expand=expand)
            else:
                out, score = temp_signature.match(signature, lambda_ = lambda_,key_match= key_match,expand=expand)
            
            if out == 0 and score == 0:
                matches.append(['',0])
            # matches.append([temp_signature.name, score,out])
            else:
